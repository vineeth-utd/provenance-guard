...
Each detection signal returns a score between 0 and 1, where a higher score means the text is more likely to be AI-generated. The final confidence score is calculated by taking the average of the LLM score and the stylometric score. Both signals contribute equally because they measure different aspects of the text.
...
The thresholds are intentionally conservative. A submission is only classified as AI-generated when the combined confidence score is high. If the signals disagree or neither signal is confident enough, the system returns an **Uncertain** label instead of making a strong claim.

This approach helps reduce false positives, since incorrectly labeling a human author's work as AI-generated can be more harmful than leaving the result uncertain. If a human-written submission is incorrectly classified as AI-generated, the creator can review the transparency label and submit an appeal with additional context. This gives creators a way to challenge the decision while keeping the system transparent.
...
Provenance Guard will expose three main endpoints.

### POST /submit

Accepts a JSON body with:
- `text`: the content to analyze
- `creator_id`: the ID of the creator who submitted the content

Returns:
- `content_id`: a unique ID for the submission
...
- `signal_scores`: the individual scores from each detection signal

### POST /appeal

Accepts a JSON body with:
- `content_id`: the ID of the original submission
- `creator_reasoning`: the creator's explanation for the appeal

Returns:
- a confirmation message
- the updated status of the content

### GET /log

Returns recent audit log entries, including:
- timestamp
- content ID
- attribution result
- confidence score
- individual signal scores
- appeal information if an appeal was submitted
